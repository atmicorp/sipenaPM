      ...
      <!-- penilaian Kelompok -->
        <div class="card card-default">
            <div class="card-header">
              <h3 class="card-title">Edit Nilai TA Individu : {{$mahasiswa->name}} ({{$mahasiswa->details->nim}})</h3>
            ...
            <div class="card-body">
                ...
                <div class="alert alert-info mb-4">
                    <strong>Dosen Penguji: </strong>{{ $penguji->userdosenTA->details->gelar_depan}} {{ $penguji->userdosenTA->name }}, {{ $penguji->userdosenTA->details->gelar_belakang}}
                </div>

                <form action="{{ route('updateNilaiIndividu', [
                ...
                <div class="form-group mb-3">
                    <label for="aspek_{{ $aspek['id_aspek_penilaian_individu'] }}" class="font-weight-bold">
                        {{ $aspek['aspek_penilaian'] }}
                    </label>

                    @if($aspek['tipedata'] === 'Deskripsi')
                        <textarea 
                            name="nilai[{{ $aspek['id_aspek_penilaian_individu'] }}]" 
                            id="aspek_{{ $aspek['id_aspek_penilaian_individu'] }}" 
                            class="form-control" 
                            rows="3"
