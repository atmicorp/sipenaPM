      ...
      <!-- penilaian Kelompok -->
        <div class="card card-default">
            <div class="card-header">
              <h3 class="card-title">Edit Nilai Magang : {{$mahasiswa->name}} ({{$mahasiswa->details->nim}})</h3>
            ...
            <div class="card-body">
                ...
                <div class="alert alert-info mb-4">
                    <strong>Dosen Penguji: </strong>{{ $penguji->userdosen->details->gelar_depan}} {{ $penguji->userdosen->name }}, {{ $penguji->userdosen->details->gelar_belakang}}
                </div>

                <form action="{{ route('updateNilaimagang', [
                      'id_penguji'   => $penguji->id,
                      'id_mahasiswa' => $mahasiswa->id,
                      'id_dosen' => $penguji->userdosen->id
                  ]) }}" method="POST">
                      @csrf
                  
                     @foreach($aspekWithNilai as $aspek)
                <div class="form-group mb-3">
                    <label for="aspek_{{ $aspek['id_aspek_penilaian_magang'] }}" class="font-weight-bold">
                        {{ $aspek['aspek_penilaian'] }}
                    </label>

                    @if($aspek['tipedata'] === 'Deskripsi')
                        <textarea 
                            name="nilai[{{ $aspek['id_aspek_penilaian_magang'] }}]" 
                            id="aspek_{{ $aspek['id_aspek_penilaian_magang'] }}" 
